...
def validate_tree_node(node, left_bound, right_bound):
    if not node:
        return True
    return left_bound <= node.data and node.data <= right_bound and \
...
class Test(unittest.TestCase):
  def test_validate_tree(self):
    ...
    self.assertEqual(validate_tree(Node(7,Node(4, None, Node(5)),Node(9))), True)
    self.assertEqual(validate_tree(Node(7,Node(4, Node(5)),Node(9))), False)
    self.assertEqual(validate_tree(Node(7,Node(4, Node(5),Node(6)),Node(9))), False)
